...
public class AdminApplicationService implements AdminService {
    ...
    private void validateCaptcha(String username, String captcha) {
        if (captcha == null || captcha.isEmpty()) {
            throw new DomainException("验证码不能为空");
        }
    ...
    public Admin createAdmin(String username, String password) {
        adminRepository.findByUsername(username)
            .ifPresent(existing -> {
                throw new DomainException("管理员已存在");
            });
    ...
    public void verifyCurrentPassword(String username, String currentPassword) {
        ...
        Admin admin = adminRepository.findByUsername(username)
            .orElseThrow(() -> new DomainException("管理员不存在"));

        if (!admin.matchPassword(currentPassword, passwordEncoder)) {
            throw new DomainException("当前密码验证失败");
        }
    }
    ...
    private void validateUsernameAndPassword(String username, String currentPassword) {
        ...
        if (currentPassword == null || currentPassword.isBlank()) {
            throw new DomainException("当前密码不能为空");
        }
    }
}
